...
# disadvantage: cannot model events that occur at the same time, exclude these
dyadicdat2 <- dyadicdat2[dyadicdat2$tstart != dyadicdat2$tstop,]
...
cox_2_tt <- coxph(Surv(tstart, time, treaty_yes) ~ agr_cum + conflict_cum + tt(conflict_cum) + monitoring_cum + commission_cum + pollution	+ shipping +	fish  + construction + symmetry + bi_lingue, cluster = dyad_id, tt = function(x, t, ...) x * log(t+20))
cox_4_tt <- coxph(Surv(tstart, time, any_mech_yes) ~ agr_cum  + conflict_cum + tt(conflict_cum) + monitoring_cum + commission_cum  + pollution	+ shipping +	fish + construction + symmetry + bi_lingue, cluster = dyad_id, tt = function(x, t, ...) x * log(t+20))
cox_6_tt <- coxph(Surv(tstart, time, commission) ~ agr_cum + tt(agr_cum) + conflict_cum + tt(conflict_cum) + monitoring_cum  + commission_cum + pollution	+ shipping + fish + construction + symmetry + bi_lingue, cluster = dyad_id, tt = function(x, t, ...) x * log(t+20))
cox_8_tt <- coxph(Surv(tstart, time, monitoring) ~ agr_cum + tt(agr_cum) + conflict_cum  + monitoring_cum  + commission_cum + pollution	+ shipping + fish + construction + symmetry + bi_lingue, cluster = dyad_id, tt = function(x, t, ...) x * log(t+20))
cox_10_tt <- coxph(Surv(tstart, time, conflict) ~ agr_cum + tt(agr_cum) + conflict_cum + monitoring_cum + commission_cum + tt(commission_cum) + pollution	+ shipping + tt(shipping) + construction + symmetry + bi_lingue, cluster = dyad_id, tt = function(x, t, ...) x * log(t+20))

model_list_tt <-  list(cox_2_tt, cox_4_tt, cox_6_tt, cox_8_tt, cox_10_tt)

library(stargazer)
stargazer(model_list_tt, type = "text", booktaps = T, title = "Cox Proportional Hazard Models: correcting for the PH Assumption",
          ...
          star.char = c("\\cdot", "*", "**", "***"), star.cutoffs = c(.1, .05, .01, .001), style = "ajs", omit.stat = c("rsq", "max.rsq", "lr", "wald", "logrank", "ll"),
