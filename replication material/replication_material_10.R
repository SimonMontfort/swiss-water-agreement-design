...
plot_dat <- dyadicdat[dyadicdat$year >= 1980 & dyadicdat$year != 0,]

# I. correlation between design principles
chi <- as.data.frame(matrix(nrow = 6, ncol = 3))
chi[1,2] <- round(chisq.test(table(plot_dat$conflict, plot_dat$monitoring))$statistic, 3)
chi[2,2] <- paste0( "(", round(chisq.test(table(plot_dat$conflict, plot_dat$monitoring))$p.value, 3), ")")
chi[1,3] <- round(chisq.test(table(plot_dat$conflict, plot_dat$commission))$statistic, 3)
chi[2,3] <- paste0( "(", round(chisq.test(table(plot_dat$conflict, plot_dat$commission))$p.value, 3), ")")
chi[3,3] <- round(chisq.test(table(plot_dat$monitoring, plot_dat$commission))$statistic, 3)
chi[4,3] <- paste0( "(", round(chisq.test(table(plot_dat$monitoring, plot_dat$commission))$p.value, 3), ")")
chi[is.na(chi)] <- "-"
...
# Robustness Checks 4: Weibull parametric model
